...
// Truy vấn tổng nhập kho và xuất kho theo từng năm
$sqlNhapKho = "
SELECT YEAR(thoigian) as Nam, tensanpham, SUM(soluong) as TongNhapKho 
FROM sanpham 
GROUP BY Nam, tensanpham";
$resultNhapKho = $mysqli->query($sqlNhapKho);

$nhapKhoData = [];
while ($row = $resultNhapKho->fetch_assoc()) {
    $nhapKhoData[$row['Nam']][$row['tensanpham']] = $row['TongNhapKho'];
}

$sqlXuatKho = "
SELECT YEAR(thoigian) as Nam, tensanpham, SUM(soluong) as TongXuatKho 
FROM xuatkho 
GROUP BY Nam, tensanpham";
...
while ($row = $resultXuatKho->fetch_assoc()) {
    $xuatKhoData[$row['Nam']][$row['tensanpham']] = $row['TongXuatKho'];
}
?>
...
<main class="main container" id="main">
    <div class="row">
        <main class="col-md-12 px-md-4">
            ...
            <!-- Biểu đồ cột -->
            <h3>Thống kê xuất/nhập kho theo năm</h3>
            <script type="text/javascript">
                ...
                function drawBarChart() {
                    ...
                    <?php
                    $sanphamList = [];
                    foreach ($nhapKhoData as $yearData) {
                        foreach (array_keys($yearData) as $sanpham) {
                            if (!in_array($sanpham, $sanphamList)) {
                                $sanphamList[] = $sanpham;
                            }
                        }
                    }
                    foreach ($xuatKhoData as $yearData) {
                        foreach (array_keys($yearData) as $sanpham) {
                            if (!in_array($sanpham, $sanphamList)) {
                                $sanphamList[] = $sanpham;
                            }
                    ...
                    $years = array_unique(array_merge(array_keys($nhapKhoData), array_keys($xuatKhoData)));
                    sort($years);
                    foreach ($years as $year) {
                        $row = ["'$year'"];
                        foreach ($sanphamList as $sanpham) {
                            $row[] = isset($nhapKhoData[$year][$sanpham]) ? $nhapKhoData[$year][$sanpham] : 0;
                            $row[] = isset($xuatKhoData[$year][$sanpham]) ? $xuatKhoData[$year][$sanpham] : 0;
                        }
                    ...
                    var options = {
                        chart: {
                            title: 'Thống kê sản phẩm theo năm',
